 ...
 */
public class TestBlocksTests {
  // +------+--------------------------------------------------------------
  // | Main |
  // +------+

  /**
   * Prints out test TextBlocks
   */
  public static void testingTextBlocks(String title, TextBlock[] testBlocks, PrintWriter pen) {
    ...
    TBUtils.print(pen, testBlocks[3]);
  ...
  public static void main(String[] args) throws Exception {
    ...
    TextBlock boxed = new BoxedBlock(new VComposition(twoLevel, new TextLine("Aloha")));

    TextBlock[] defBlocks = { block, twoLevel, threeLevel, boxed };
    ...
    TextBlock[] truncated3Blocks = { new Truncated(block, 0), new Truncated(twoLevel, 0), new Truncated(threeLevel, 0),
        new Truncated(boxed, 0) };
    testingTextBlocks("Truncated to Width 0", truncated3Blocks, pen);

    // Centered [width 4]
    TextBlock[] centeredBlocks = { new Centered(block, 4), new Centered(twoLevel, 4), new Centered(threeLevel, 4),
        new Centered(boxed, 4) };
    ...
    TextBlock[] centered2Blocks = { new Centered(block, 8), new Centered(twoLevel, 8), new Centered(threeLevel, 8),
        new Centered(boxed, 8) };
    testingTextBlocks("Centered to Width 8", centered2Blocks, pen);

    // Centered [width 4]
    TextBlock[] rightBlocks = { new RightJustified(block, 4), new RightJustified(twoLevel, 4),
        new RightJustified(threeLevel, 4), new RightJustified(boxed, 4) };
    testingTextBlocks("Right Justified to Width 4", rightBlocks, pen);

    // Centered [width 8]
    TextBlock[] right2Blocks = { new RightJustified(block, 8), new RightJustified(twoLevel, 8),
        new RightJustified(threeLevel, 8), new RightJustified(boxed, 8) };
    testingTextBlocks("Right Justified to Width 8", right2Blocks, pen);

    // Horizontally Flipped
    TextBlock[] horizontalBlocks = { new HorizontallyFlipped(block), new HorizontallyFlipped(twoLevel),
        new HorizontallyFlipped(threeLevel), new HorizontallyFlipped(boxed) };
    testingTextBlocks("Horizontally Flipped", horizontalBlocks, pen);

    // Vertically Flipped
    TextBlock[] verticalBlocks = { new VerticallyFlipped(block), new VerticallyFlipped(twoLevel),
        new VerticallyFlipped(threeLevel), new VerticallyFlipped(boxed) };
